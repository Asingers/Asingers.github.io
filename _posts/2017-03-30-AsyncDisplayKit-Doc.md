...
垂直	`constrainedSize`被解释为具有无限的`.height`（`CGFLOAT_MAX`），允许布局规范中的堆栈和其他内容扩展并产生可滚动的内容。
水平	`constrainedSize`被解释为具有无限制的`.width`（`CGFLOAT_MAX`）。
垂直水平	`constrainedSize`被解释为双向无界。
例
